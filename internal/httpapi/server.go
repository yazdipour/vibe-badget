...
func NewServer(s *store.Store, static fs.FS) http.Handler {
	...
	srv.mux.HandleFunc("PUT /api/categories/{id}/kind", srv.updateCategoryKind)
	srv.mux.HandleFunc("PUT /api/categories/{id}/name", srv.updateCategoryName)
	srv.mux.HandleFunc("DELETE /api/categories/{id}", srv.deleteCategory)
	...
	srv.mux.HandleFunc("DELETE /api/accounts/{id}", srv.deleteAccount)
